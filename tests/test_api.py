...
headers = {
    ...
    "authorization":"Bearer eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9.eyJpc3MiOiJodHRwczovL3VzZXItcmlnaHQiLCJzdWIiOjIyMDYwODYwLCJpYXQiOjE3NDkwNjIwNjMsImV4cCI6MTc0OTA2NTY2MywidHlwZSI6MjB9.wUsxvp85D9ryit88DcEl6igEr2CjJbHbIVUgI74Ycc4"
}
...
def test_api_book_by_author():
    resp = requests.get(f"{base_url}search/product?phrase=булгаков", headers=headers)
...
def test_api_book_by_title():
    resp = requests.get(f"{base_url}search/product?phrase=мобби дик", headers=headers)
    assert resp.status_code == 200
...
def test_search_by_language_english():
    resp = requests.get(f"{base_url}search/product?phrase=Moby Dick", headers=headers)
    assert resp.status_code == 200
...
@allure.epic("API Тестирование")
@allure.feature("Поиск книг")
@allure.title("Тестирование поиска отсутствием авторизации")
@allure.description("Проверка, что API возвращает ошибку при отсутствии авторизации.")
def test_negative_api_empty_search():
    resp = requests.get(f"{base_url}search/product?phrase=Moby Dick")
